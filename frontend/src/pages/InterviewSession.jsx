...
export default function InterviewSession() {
  ...
  const handleAutoSubmit = async () => { if (recording) stopRecording(); await submitAnswer(true); };
  ...
  const submitAnswer = async (isAuto = false) => {
    ...
    try {
      ...
      if (settings.showScoreOnSubmit && res.data.data.score !== undefined) {
        setLastScore(res.data.data.score); setShowScore(true);
        setTimeout(() => setShowScore(false), 2000);
      }
      setQuestion(res.data.data.question.text);
      setCurrentIndex(res.data.data.currentIndex);
      setAudioBlob(null); setTimeLeft(40); warnedRef.current = false;
    } catch (err) { console.error(err); }
    finally { setLoading(false); }
  };

  const timerPercent  = (timeLeft / 40) * 100;
  const timerColor    = timeLeft > settings.timerAmberAt ? "text-emerald-400" : timeLeft > settings.timerRedAt ? "text-amber-400" : "text-red-400";
    ...
    <div className="min-h-screen bg-[#09090b] flex items-center justify-center px-4">
      ...
      <div className="w-full max-w-2xl relative">
        <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-8 shadow-2xl backdrop-blur-sm">
            ...
            <div className="flex items-center gap-3">
              <div className="flex gap-1.5 items-center">
                {Array.from({ length: total }).map((_, i) => (
                  <div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${i < currentIndex ? "w-4 bg-violet-500" : i === currentIndex ? "w-4 bg-violet-400 shadow-[0_0_8px_rgba(167,139,250,0.8)]" : "w-1.5 bg-white/10"}`} />
                ))}
              </div>
          ...
          {driveId && (
            <div className="flex items-center gap-2 mb-5 px-3 py-1.5 bg-violet-600/10 border border-violet-500/20 rounded-lg w-fit">
              <span className="w-1.5 h-1.5 rounded-full bg-violet-400" />
              <span className="text-[11px] text-violet-300/70 font-medium">Campus Drive · Round in progress</span>
            </div>
          )}
          ...
          <div className="mb-7">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className={`text-sm font-mono font-bold tabular-nums ${timerColor}`}>{String(timeLeft).padStart(2, "0")}s</span>
            </div>
